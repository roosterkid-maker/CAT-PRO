...
export interface PaperTradingReadiness {
  generatedAt: number;
  version: "20.9";
  mode: "READ_ONLY_PAPER_READINESS";
  evidenceStatus: "AVAILABLE";
  stage: PaperTradingReadinessStage;
  readyForShadowDeployment: boolean;
  ...
  soak: {
    ...
    attributedPaperTrades: number | null;
    attributedClosedTrades: number | null;
    remainingAttributedClosedTrades: number | null;
    attributedNetProfit: number | null;
    minimumConsecutiveRuntimePasses: number;
    consecutiveRuntimePasses: number;
    remainingConsecutiveRuntimePasses: number;
    status:
      | "NOT_STARTED"
      | "COLLECTING"
      | "READY_FOR_DEPLOYMENT_REVIEW";
  };
  gates: PaperTradingReadinessGate[];
  blockers: string[];
  notes: string[];
}

export interface PaperTradingReadinessResponse {
  success: true;
  data: PaperTradingReadiness;
}
