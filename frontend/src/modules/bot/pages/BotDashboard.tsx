    ...
    <div className="capital-manager-view space-y-5">
      ...
      <article className="overflow-hidden rounded-2xl border border-border-default bg-panel">
        ...
        <div className="grid grid-cols-2 gap-px bg-border-default lg:grid-cols-4">
          ...
          <FocusHeadline label="Withdrawable LIVE profit" value="NO DATA" detail="requires reconciled LIVE ledger" />
        </div>
        <div className="grid gap-px border-t border-border-default bg-border-default md:grid-cols-3 2xl:grid-cols-6">
          ...
          <CapitalTruthMetric label="TDS withheld" value={formatWholeRupees(manager.profitTruth.tdsWithheldInr)} />
      ...
      <article className="overflow-hidden rounded-2xl border border-border-default bg-panel">
        <PanelHeader icon={<Target className="size-4" />} eyebrow="PHASE B · REQUIREMENT-DRIVEN ALLOCATION" title="Dynamic targets—not equal allocation" right={<span className={`rounded-full border px-2.5 py-1 font-mono text-[8px] font-bold ${manager.allocation.status === "TARGETS_AVAILABLE" ? "border-emerald-400/25 bg-emerald-400/10 text-emerald-300" : "border-amber-400/25 bg-amber-400/10 text-amber-300"}`}>{manager.allocation.status.replaceAll("_", " ")}</span>} />
...
function CircularGauge({
  ...
  const colors = {
    cyan: {stroke: "#22d3ee", text: "text-cyan-200"},
    green: {stroke: "#19f6a5", text: "text-emerald-300"},
    violet: {stroke: "#a855f7", text: "text-violet-300"},
  } as const;
...
function StrategyOneTinyLiveOpportunityAuditPanel({
  ...
  const stateTone = audit?.state === "READY_FOR_POLICY_REVIEW"
  ...
  return (
    <article className="overflow-hidden rounded-2xl border border-cyan-300/18 bg-panel">
      <PanelHeader
        ...
        eyebrow="V126.1 READ-ONLY TINY-LIVE AUDIT"
        title="Why no real order yet"
        right={(
          <div className="flex items-center gap-2">
            <span className={`rounded-full border px-2.5 py-1 font-mono text-[9px] font-bold ${stateTone}`}>
              {loading ? "LOADING" : audit?.state.replaceAll("_", " ") ?? "NO DATA"}
            </span>
            <button
              type="button"
              onClick={onRefresh}
      ...
      {audit ? (
        <>
          <div className="grid gap-px bg-border-default sm:grid-cols-2 xl:grid-cols-5">
            ...
            />
            <ActivityMetric
              label={`Qualified ≥${audit.thresholds.qualificationNetProfitPercent.toFixed(2)}%`}
              value={formatInteger(audit.observation.profitBands.qualified)}
              detail="below LIVE minimum"
              tone="warning"
            />
            <ActivityMetric
              label={`LIVE economics ≥${audit.thresholds.liveNetProfitPercent.toFixed(2)}%`}
              value={formatInteger(audit.observation.profitBands.liveEligible)}
              detail="economics only · not authorization"
              tone={audit.observation.profitBands.liveEligible > 0 ? "positive" : "warning"}
            />
            <ActivityMetric
              label="Dispatch-ready LIVE"
              value={formatInteger(audit.observation.dispatchReservedLiveEligibleGenerations)}
              detail={`fresh books ≤${formatInteger(audit.thresholds.dispatchReservedMaximumBookAgeMs)} ms`}
          ...
          <div className="grid gap-px bg-border-default xl:grid-cols-[1.2fr_.8fr]">
            <div className="min-w-0 bg-panel px-5 py-5">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  <p className="font-mono text-[9px] font-bold tracking-[0.13em] text-cyan-200">BINANCE ↔ BYBIT ROUTE RANKING</p>
              ...
              {topRoutes.length > 0 ? (
                ...
                <EmptyState title="Collecting fresh economics" detail="This deployment starts a truthful economics cohort from new unique Binance/Bybit quote generations; no historical profit sample is fabricated." />
              )}
            </div>
