              ...
              <div className="min-w-[820px]">
                ...
                {report.currentCandidates.slice(0, 8).map((candidate) => (
                  <div key={candidate.opportunityId} className="grid grid-cols-[1fr_1.15fr_.6fr_.8fr_.8fr_1.4fr] items-center gap-3 border-b border-border-default/70 py-2.5 text-xs last:border-0">
                    <div><p className="font-mono font-bold text-text-primary">{candidate.market}</p><p className={`mt-0.5 font-mono text-[9px] ${candidate.selectableForPaper ? "text-emerald-300" : "text-cyan-300"}`}>ENGINE {candidate.decision} · {candidate.paperSelectionState === "SELECTABLE" ? "PAPER SELECTABLE" : "NO ORDER"}</p></div>
                    <div className="min-w-0"><p className="truncate font-semibold capitalize text-text-primary">{candidate.buyExchange} → {candidate.sellExchange}</p><p className="mt-0.5 font-mono text-[8px] text-text-muted">OBS {candidate.consecutiveObservations} · {candidate.persistenceMs} ms</p><p className={`mt-0.5 font-mono text-[8px] ${candidate.paperAdmissionAllowed ? "text-text-muted" : "text-red-300"}`}>SETTLED HISTORY {candidate.routeProfitState} · {candidate.routeSampleTrades} trades</p></div>
                    ...
                    <div><p className="font-mono text-[9px] font-bold text-brand">{candidate.currentStage.replaceAll("_", " ")}</p><p className="mt-0.5 font-mono text-[8px] text-text-muted">{candidate.qualificationStatus}</p>{candidate.failedChecks.length > 0 ? <p className="mt-0.5 font-mono text-[8px] text-red-300">FAIL: {candidate.failedChecks.join(", ")}</p> : null}</div>
