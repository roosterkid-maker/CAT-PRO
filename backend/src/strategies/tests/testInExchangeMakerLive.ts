...
function harness(directory: string, name: string, overrides: {
  ...
  const book = (market: string) => ({
    exchange: "coindcx", market, timestamp: now - 100,
  ...
  return {engine, calls, published, advance: (ms: number) => { now += ms; }};
}
...
async function testAttempts(directory: string): Promise<void> {
  ...
  assert.match(await cross.engine.attempt("RWA", "BUY"), /^WOULD_CROSS/u);
  ...
  const quiet = harness(directory, "quiet", {quote: {bid: null, ask: 0.20149}});
