...
function path(input: {
  ...
  return {
    ...
    expectedNetProfitQuantity: input.netPercent === null ? null : initial * input.netPercent / 100,
    expectedNetProfitPercent: input.netPercent,
    netProfitQuantity: input.netPercent === null ? null : initial * input.netPercent / 100,
    netProfitPercent: input.netPercent,
    stressNetProfitQuantity: input.netPercent === null ? null : initial * input.netPercent / 100,
    stressNetProfitPercent: input.netPercent,
    absoluteNetProfitInr: input.netPercent === null ? null : initial * input.netPercent / 100 * 85,
    startAssetInrValue: 85,
    tdsCapitalLockInr: 85,
...
const simulation: TriangularArbitrageSimulationSnapshot = {
  ...
  evaluatedPaths: 2,
  fastScreenPassedPaths: 2,
  fullDepthEvaluatedPaths: 2,
  qualifiedPaths: 0,
  blockedPaths: 2,
  simulations: [
  ...
  safety: {
    shadowOnly: true,
    paperExecutionImplemented: true,
