...
import {PERPETUAL_PERPETUAL_ARBITRAGE_STRATEGY_ID} from "../models/StrategyMetadata";
...
import type {PerpetualPerpetualArbitrageStrategySignal, StrategySignal} from "../models/StrategySignal";
import {
...
import type {
  ...
  PerpetualPerpetualArbitrageConfigurationInput,
} from "./PerpetualPerpetualArbitrageConfiguration";
import {PerpetualPerpetualArbitrageEconomicsEngine} from "./PerpetualPerpetualArbitrageEconomicsEngine";
import type {PerpetualPerpetualArbitrageEconomicsSnapshot} from "./PerpetualPerpetualArbitrageEconomicsEngine";

export interface PerpetualPerpetualArbitrageMarketSource {
...
const METADATA: StrategyMetadata = {
  ...
  strategyNumber: 6,
  displayName: "Perpetual-Perpetual Arbitrage",
  version: "29.0",
  category: "PERPETUAL_PERPETUAL_ARBITRAGE",
  description: "SHADOW-only cross-venue perpetual dislocation economics with full depth, round-trip fees and adverse-funding reserve.",
...
export class PerpetualPerpetualArbitrageStrategyController implements StrategyController {
  private readonly configuration: PerpetualPerpetualArbitrageConfiguration;
  private readonly listeners = new Set<StrategySignalListener>();
  private currentSignals: readonly PerpetualPerpetualArbitrageStrategySignal[] = [];
  private latestEconomics: PerpetualPerpetualArbitrageEconomicsSnapshot | null = null;
  ...
  constructor(
    ...
    private readonly marketSource: PerpetualPerpetualArbitrageMarketSource = derivativeMarketDataService,
    private readonly economicsEngine = new PerpetualPerpetualArbitrageEconomicsEngine(),
  ) { this.configuration = createPerpetualPerpetualArbitrageConfiguration(input); }

  getMetadata(): StrategyMetadata { return structuredClone(METADATA); }
  getConfiguration(): PerpetualPerpetualArbitrageConfiguration { return this.configuration; }
  start(): void {
  ...
  stop(): void {
    ...
    this.unsubscribeMarketData?.(); this.unsubscribeMarketData = null; this.currentSignals = [];
    this.running = false; this.stopCount += 1; this.lastStoppedAt = Date.now();
  }
  isRunning(): boolean { return this.running; }
  getEconomicsSnapshot(): PerpetualPerpetualArbitrageEconomicsSnapshot | null { return this.latestEconomics ? immutableClone(this.latestEconomics) : null; }
  getDiagnosticEvidence(): unknown { return this.getEconomicsSnapshot(); }
  getRuntimeSnapshot(now = Date.now()): StrategyRuntimeSnapshot {
    const signals = this.getSignals(now);
    return {
      strategyId: PERPETUAL_PERPETUAL_ARBITRAGE_STRATEGY_ID, generatedAt: now, running: this.running,
      ...
      evidence: {snapshot: this.latestEconomics ? "AVAILABLE" : "NO_DATA", signals: signals.length > 0 ? "AVAILABLE" : "NO_DATA", performance: "NOT_REPORTED"},
      legacyHistoryAttribution: "UNATTRIBUTED_LEGACY",
  ...
  getSignals(now = Date.now()): readonly StrategySignal[] {
    return this.currentSignals.filter((signal) => signal.expiresAt >= now).map((signal) => immutableStrategySignal(signal));
  }
    ...
    try {
      ...
      const signals = economics.assessments
        ...
        .map((assessment) => immutableStrategySignal({
          id: `${PERPETUAL_PERPETUAL_ARBITRAGE_STRATEGY_ID}:${assessment.id}`,
          strategyId: PERPETUAL_PERPETUAL_ARBITRAGE_STRATEGY_ID,
          kind: "PERPETUAL_PERPETUAL_ARBITRAGE_SHADOW_OPPORTUNITY",
          evidenceStatus: "AVAILABLE", source: "DerivativeMarketData",
          ...
          automaticExecutionAllowed: false, evidence: assessment.evidence!,
        }) as PerpetualPerpetualArbitrageStrategySignal);
      this.latestEconomics = economics; this.currentSignals = signals; this.processedSnapshots += 1;
      this.totalSignalsObserved += signals.length; this.lastSnapshotGeneratedAt = snapshot.generatedAt;
      this.lastSnapshotReceivedAt = receivedAt; this.lastSnapshotOpportunityCount = economics.qualifiedRoutes;
      ...
      this.currentSignals = []; this.lastError = error instanceof Error ? error.message : "Unknown perpetual-perpetual arbitrage error.";
    }
  }
}
