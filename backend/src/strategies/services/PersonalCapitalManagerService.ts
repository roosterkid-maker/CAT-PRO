...
import {
  ...
  type CapitalAllocationAndImbalanceReport,
...
export interface PersonalCapitalManagerInput {
  ...
  readonly paperCapital: {
    ...
    readonly tdsReceivableInr: number;
  };
  ...
  readonly normalizedInventory: NormalizedInventorySnapshot;
  readonly rebalancingSafetyContext: {
...
export interface PersonalCapitalManagerRebalancingReport {
  readonly version: "158.0";
  readonly phase: "PHASE_A_B_ADVISORY";
  readonly authorityMode: "ADVISORY_ONLY";
  readonly inventory: {
    readonly version: "121.0";
    readonly generatedAt: number;
    readonly state: NormalizedInventorySnapshot["state"];
    readonly valuationAsset: "USDT";
    readonly maximumBalanceAgeMs: number;
    readonly totals: NormalizedInventorySnapshot["totals"];
    readonly exchanges: readonly {
      ...
      readonly unvaluedPositiveAssets: readonly string[];
    }[];
    readonly blockers: readonly string[];
    readonly limitations: readonly string[];
    readonly missingValuesTreatedAsZero: false;
    readonly accountingCapitalMixedWithWalletValuation: false;
  };
  readonly policyBasis: {
    readonly policyId: string;
    readonly revision: number;
    readonly source: "CREDIBLE_STRATEGY_ONE_SETTLEMENTS_PLUS_CURRENT_ROUTE";
    readonly staticEqualAllocationUsed: false;
    readonly crediblePaperSettlements: number;
    readonly currentRouteBoostApplied: boolean;
    readonly evidenceSufficient: boolean;
    readonly formula: string;
  };
  readonly allocation: CapitalAllocationAndImbalanceReport;
  readonly plan: RebalancingDecisionPlan;
  readonly safetyContext: PersonalCapitalManagerInput["rebalancingSafetyContext"];
  readonly phases: {
    readonly phaseAUnifiedTruth: "ACTIVE";
    readonly phaseBAdvisoryRebalancing: "ACTIVE";
    readonly phaseCManualTransfers: "LOCKED_NOT_IMPLEMENTED";
    readonly phaseDCappedAutomaticTransfers: "LOCKED_NOT_IMPLEMENTED";
    readonly phaseEProfitWithdrawalManager: "LOCKED_WITHOUT_LIVE_LEDGER";
  };
  readonly safety: {
    readonly readOnly: true;
    readonly executionHotPathUntouched: true;
    ...
    readonly explicitAuthorityRequiredForLaterPhases: true;
  };
}
...
export interface PersonalCapitalManagerReport {
  ...
  readonly capitalTruth: {
    ...
    readonly paper: {
      ...
      readonly availableAccountingEquityInr: number;
      readonly tdsReceivableInr: number;
      readonly tdsTreatment: "RECOVERABLE_CASH_LOCK_NOT_TRADING_FEE";
  ...
  readonly rebalancing: PersonalCapitalManagerRebalancingReport;
  readonly safety: {
...
export class PersonalCapitalManagerService {
  getReport(input: PersonalCapitalManagerInput): PersonalCapitalManagerReport {
    ...
    const rebalancing = buildIntegratedRebalancing(input);

    return deepFreeze({
      version: "158.0" as const,
...
function buildCapitalTruth(
  ...
  return {
    ...
    paper: {
      ...
      tdsReceivableInr: round(input.paperCapital.tdsReceivableInr),
...
function buildIntegratedRebalancing(
  input: PersonalCapitalManagerInput,
): PersonalCapitalManagerRebalancingReport {
  const policyResult = buildDynamicCapitalAllocationPolicy(
    input.capitalPlacement,
    input.inventoryPlan.recommendedRoute,
    input.normalizedInventory,
  );
  const allocation = capitalAllocationAndImbalanceService.evaluate(
    input.normalizedInventory,
    policyResult.policy,
    input.now,
  );
  const naturalRebalanceCandidateKeys = allocation.state === "READY"
    ? buildNaturalRebalanceCandidateKeys(
        allocation,
        input.inventoryPlan.recommendedRoute,
      )
    : [];
  const plan = rebalancingDecisionEngine.plan(
    allocation,
    {
      naturalRebalanceCandidateKeys,
      ...input.rebalancingSafetyContext,
    },
    undefined,
    input.now,
  );
  ...
  return deepFreeze({
    version: "158.0" as const,
    phase: "PHASE_A_B_ADVISORY" as const,
    authorityMode: "ADVISORY_ONLY" as const,
    inventory: {
      version: input.normalizedInventory.version,
      generatedAt: input.normalizedInventory.generatedAt,
      state: input.normalizedInventory.state,
      valuationAsset: input.normalizedInventory.valuationAsset,
      maximumBalanceAgeMs: input.normalizedInventory.maximumBalanceAgeMs,
      totals: structuredClone(input.normalizedInventory.totals),
      exchanges: input.normalizedInventory.exchanges.map((exchange) => ({
        exchange: exchange.exchange,
        displayName: exchange.displayName,
        balanceStatus: exchange.balanceStatus,
        lastSynchronizedAt: exchange.lastSynchronizedAt,
        balanceAgeMs: exchange.balanceAgeMs,
        positiveAssets: exchange.totals.positiveAssets,
        currentValuations: exchange.totals.currentValuations,
        staleValuations: exchange.totals.staleValuations,
        unavailableValuations: exchange.totals.unavailableValuations,
        authoritativeTotalValueUsdt:
          exchange.totals.authoritativeTotalValueUsdt,
        authoritativeAvailableAfterReservationsValueUsdt:
          exchange.totals.authoritativeAvailableAfterReservationsValueUsdt,
        knownTotalValueUsdt: exchange.totals.knownTotalValueUsdt,
        directUsdtAvailableAfterReservations:
          exchange.totals.directUsdtAvailableAfterReservations,
        unvaluedPositiveAssets: exchange.assets
          .filter((asset) =>
            asset.totalBalance > 0 &&
            asset.valuation.state === "UNAVAILABLE")
          .map((asset) => asset.asset)
          .sort(),
      })),
      blockers: [...input.normalizedInventory.blockers],
      limitations: [...input.normalizedInventory.limitations],
      missingValuesTreatedAsZero: false as const,
      accountingCapitalMixedWithWalletValuation: false as const,
    },
    policyBasis: {
      policyId: policyResult.policy.policyId,
      revision: policyResult.policy.revision,
      source: "CREDIBLE_STRATEGY_ONE_SETTLEMENTS_PLUS_CURRENT_ROUTE" as const,
      staticEqualAllocationUsed: false as const,
      crediblePaperSettlements:
        input.capitalPlacement.evidence.credibleSettlements,
      currentRouteBoostApplied: policyResult.currentRouteBoostApplied,
      evidenceSufficient: policyResult.evidenceSufficient,
      formula:
        "Venue score = credible BUY/SELL settlement share × confidence × win-rate × profitability; a bounded fresh-route boost is added, then all five targets are normalized to 100%.",
    },
    allocation,
    plan,
    safetyContext: {...input.rebalancingSafetyContext},
    phases: {
      phaseAUnifiedTruth: "ACTIVE" as const,
      phaseBAdvisoryRebalancing: "ACTIVE" as const,
      phaseCManualTransfers: "LOCKED_NOT_IMPLEMENTED" as const,
      phaseDCappedAutomaticTransfers: "LOCKED_NOT_IMPLEMENTED" as const,
      phaseEProfitWithdrawalManager: "LOCKED_WITHOUT_LIVE_LEDGER" as const,
    },
    safety: {
      readOnly: true as const,
      executionHotPathUntouched: true as const,
      ...
      explicitAuthorityRequiredForLaterPhases: true as const,
    },
  });
}

function buildDynamicCapitalAllocationPolicy(
  placement: StrategyOneCapitalPlacementReport,
  route: PersonalCapitalManagerInventoryRouteInput | null,
  ...
  const exchangeIds = inventory.exchanges
    .map((exchange) => exchange.exchange.trim().toLowerCase())
    .sort();
  const scores = new Map(exchangeIds.map((exchange) => [exchange, 0]));

  for (const venue of [...placement.buyVenues, ...placement.sellVenues]) {
    const exchange = venue.exchange.trim().toLowerCase();
    if (!scores.has(exchange)) continue;
    const confidenceWeight = venue.confidence === "HIGH"
      ? 1
      : venue.confidence === "MEDIUM"
        ? 0.75
        : 0.5;
    const winRateWeight = Math.max(0.25, venue.winRatePercent / 100);
    const profitabilityWeight = venue.realizedPnlInr > 0 ? 1 : 0.5;
    const score = venue.settlementSharePercent * confidenceWeight *
      winRateWeight * profitabilityWeight;
    scores.set(exchange, (scores.get(exchange) ?? 0) + score);
  }

  const historicalScore = [...scores.values()].reduce(
    (total, score) => total + score,
    0,
  );
  const routeExchanges = route === null
    ? []
    : [...new Set([
        route.buyExchange.trim().toLowerCase(),
        route.sellExchange.trim().toLowerCase(),
      ])].filter((exchange) => scores.has(exchange));
  if (routeExchanges.length > 0) {
    const totalRouteBoost = Math.max(20, historicalScore * 0.25);
    const perExchangeBoost = totalRouteBoost / routeExchanges.length;
    for (const exchange of routeExchanges) {
      scores.set(exchange, (scores.get(exchange) ?? 0) + perExchangeBoost);
    }
  }

  const evidenceSufficient = historicalScore > 0 || routeExchanges.length > 0;
  if (!evidenceSufficient) {
    return {
      policy: {
        policyId: "cat-pro-strategy-one-demand-allocation-v2-no-evidence",
        revision: 2,
        softImbalancePercent: 10,
        hardImbalancePercent: 20,
        criticalImbalancePercent: 35,
        targets: exchangeIds.map((exchange) => ({
          exchange,
          targetPercent: 0,
          minimumPercent: 0,
          maximumPercent: 0,
      ...
      currentRouteBoostApplied: false,
    };
  }

  const scoreTotal = [...scores.values()].reduce(
    (total, score) => total + score,
    0,
  );
  const rawTargets = exchangeIds.map((exchange) => ({
    exchange,
    targetPercent: scoreTotal > 0
      ? (scores.get(exchange) ?? 0) / scoreTotal * 100
      : 0,
  }));
  const roundedTargets = rawTargets.map((target) => ({
    ...target,
    targetPercent: roundPercent(target.targetPercent),
  }));
  const roundedTotal = roundedTargets.reduce(
    (total, target) => total + target.targetPercent,
    0,
  );
  const highestTarget = roundedTargets.reduce((best, target) =>
    target.targetPercent > best.targetPercent ? target : best,
  );
  highestTarget.targetPercent = roundPercent(
    highestTarget.targetPercent + (100 - roundedTotal),
  );
  const totalCapitalUsdt = inventory.totals.authoritativeTotalCapitalUsdt ??
    inventory.totals.knownTotalValueUsdt;
  const emergencyReserveUsdt = round(Math.max(0, totalCapitalUsdt * 0.01));

  return {
    policy: {
      policyId: "cat-pro-strategy-one-demand-allocation-v2",
      revision: 2,
      softImbalancePercent: 10,
      hardImbalancePercent: 20,
      criticalImbalancePercent: 35,
      targets: roundedTargets.map((target) => ({
        exchange: target.exchange,
        targetPercent: target.targetPercent,
        minimumPercent: roundPercent(target.targetPercent * 0.5),
        maximumPercent: roundPercent(Math.min(
          100,
          Math.max(target.targetPercent, target.targetPercent * 1.5 + 5),
        )),
        emergencyReserveUsdt,
      })),
    },
    evidenceSufficient: true,
    currentRouteBoostApplied: routeExchanges.length > 0,
  };
}

function buildNaturalRebalanceCandidateKeys(
  allocation: CapitalAllocationAndImbalanceReport,
  route: PersonalCapitalManagerInventoryRouteInput | null,
): string[] {
  if (route === null || route.fundingState === "BLOCKED") return [];
  const buyVenue = allocation.exchanges.find(
    (exchange) => exchange.exchange === route.buyExchange.trim().toLowerCase(),
  );
  const sellVenue = allocation.exchanges.find(
    (exchange) => exchange.exchange === route.sellExchange.trim().toLowerCase(),
  );
  if (!buyVenue || !sellVenue) return [];
  const buyHasSurplus = buyVenue.imbalanceUsdt > 0;
  const sellHasDeficit = sellVenue.imbalanceUsdt < 0;
  return buyHasSurplus && sellHasDeficit ? [route.routeKey] : [];
}
...
function validateCapitalManagerInput(input: PersonalCapitalManagerInput): void {
  const nonNegativeValues = [
    ...
    input.paperCapital.tdsReceivableInr,
