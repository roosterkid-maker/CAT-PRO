...
export class StrategyOneTinyLiveOpportunityAuditService {
  ...
  getReport(now = Date.now()): StrategyOneTinyLiveOpportunityAuditReport {
    ...
    const profitBands = routes.reduce((total, route) => ({
      discovered: total.discovered + route.economics.profitBands.discovered,
      qualified: total.qualified + route.economics.profitBands.qualified,
      liveEligible: total.liveEligible + route.economics.profitBands.liveEligible,
...
function rankRoutes(
  ...
  return routes
    .map((route) => ({route, blocker: dominantBlocker(route)}))
    .sort((first, second) =>
      ...
      second.route.economics.profitBands.liveEligible - first.route.economics.profitBands.liveEligible ||
    ...
    .map(({route, blocker}, index) => ({
      ...
      liveEligibleGenerations: route.economics.profitBands.liveEligible,
      qualifiedGenerations: route.economics.profitBands.qualified,
      discoveredGenerations: route.economics.profitBands.discovered,
      dispatchReservedLiveEligibleGenerations:
...
function blockerRanking(
  ...
  for (const route of routes) {
    add("PROFIT_BELOW_LIVE_MINIMUM",
      route.economics.profitBands.discovered + route.economics.profitBands.qualified,
      "Fee-adjusted net return remained below the unchanged Tiny-LIVE minimum.");
