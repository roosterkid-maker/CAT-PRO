...
export class StrategyOneTwoLegLiveExecutionService {
  ...
  private validatePair(
    ...
    const exchanges =
      new Set([normalize(buy.exchange), normalize(sell.exchange)]);

    if (
      buy.side !== "buy" ||
      sell.side !== "sell" ||
      normalizeMarket(buy.market) !== normalizeMarket(sell.market) ||
      exchanges.size !== 2 ||
      ...
      throw new Error(
        "Strategy #1 two-leg LIVE pair requires exact Binance/Bybit SPOT limit-FOK requests with equal positive quantity and durable client IDs.",
      );
    }
  }
