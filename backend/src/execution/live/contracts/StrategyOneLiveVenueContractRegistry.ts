...
const STATIC_EVIDENCE:
  ...
  >> = Object.freeze({
    ...
    coindcx: {
      ...
      deterministicClientOrderIdentity: "DOCUMENTED_NOT_MAPPED",
...
export class StrategyOneLiveVenueContractRegistry {
  ...
  getVenue(
    ...
    const supportsFok =
      evidence.documentedTimeInForce.includes("FOK") &&
      evidence.exactFokAdapterMapping;
    const runtimePrivateFillSessionReady =
      normalized === "binance" || normalized === "bybit"
        ? this.dependencies.isPrivateFillSessionReady(normalized)
        : false;
    const calibratedOrderSubmissionTtlMs =
      route && evidence.classification === "SAFE_PILOT_CANDIDATE"
    ...
    if (!supportsFok) {
      blockers.push("AUDITED_SPOT_FOK_CONTRACT_UNAVAILABLE");
    }
