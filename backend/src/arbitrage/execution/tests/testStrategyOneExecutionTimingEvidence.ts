...
function rawRouteSnapshot(generatedAt: number): OpportunitySnapshot {
...
function main(): void {
  ...
  try {
    ...
    const rawService = new StrategyOneExecutionTimingEvidenceService({filePath: rawFilePath,
      maximumRoutes: 2, maximumSamplesPerMetric: 2, maximumOpportunitiesPerSnapshot: 4,
      persistenceIntervalMs: 60_000, maximumPersistedSnapshots: 2,
      minimumRouteSampleIntervalMs: 1, minimumPublicSamples: 2,
      minimumPrivateFillSamplesPerVenue: 2, minimumObservationSpanMs: 1_000});
    for (const generatedAt of [NOW + 10_000, NOW + 11_000]) {
      const value = rawRouteSnapshot(generatedAt);
      rawService.observePaperStage(value, "PIPELINE_START", generatedAt + 5);
      rawService.observePaperStage(value, "QUEUE_READY", generatedAt + 7);
      rawService.observePaperStage(value, "EXECUTION_START", generatedAt + 9);
      rawService.observePaperStage(value, "EXECUTION_COMPLETE", generatedAt + 13);
    }
    const rawReport = rawService.getReport(NOW + 11_100);
    assert.equal(rawReport.routes[0]?.routeKey, "BBUSDT:coindcx->binance");
    assert.equal(rawReport.routes[0]?.paperSnapshots, 2,
      "Executable pilot books must produce route timing without a profitable opportunity.");
