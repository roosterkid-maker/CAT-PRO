...
function main(): void {
  ...
  try {
    ...
    assert.equal(reviewableAudit.observation.profitBands.qualified, 1);
    assert.equal(reviewableAudit.observation.profitBands.liveEligible, 4);
    assert.equal(reviewableAudit.blockerRanking.find(
      (blocker) => blocker.code === "PROFIT_BELOW_LIVE_MINIMUM")?.count, 2);
  ...
  console.log("V126.1 exact Binance/Bybit evidence preserves wall-clock and event-time truth, ranks read-only Tiny-LIVE blockers and grants no LIVE authority.");
}
