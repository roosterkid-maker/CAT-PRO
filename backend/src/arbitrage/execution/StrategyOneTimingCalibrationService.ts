...
const BOOTSTRAP_DISPATCH_SAFETY_MARGIN_MS = 10;
const MINIMUM_CALIBRATED_BOOK_AGE_MS = 25;
const REQUIRED_OPERATIONAL_HEADROOM_MS = 10;
...
export class StrategyOneTimingCalibrationService {
  ...
  constructor(
    ...
    private readonly maximumApprovalDurationMs = 60 * 60 * 1_000,
  ...
  reviewHeadroom(
    ...
    if (!isExactStrategyOnePilotRoute({buyExchange, sellExchange})) {
      blockers.push(
        "Timing headroom review is restricted to the exact Binance/Bybit SPOT pilot lane.",
      );
    }
    ...
    const dispatchBudgetMs =
      decisionToStartP99 === null
        ? null
        : Math.ceil(
            decisionToStartP99 + BOOTSTRAP_DISPATCH_SAFETY_MARGIN_MS,
          );
    ...
    return freeze({
      ...
      decisionToExecutionStartP99Ms: decisionToStartP99,
      dispatchBudgetMs,
      maximumBookAgeMs,
  ...
  propose(
    ...
    if (!isExactStrategyOnePilotRoute(route)) {
      throw new Error(
        "Strategy #1 timing calibration is restricted to the exact Binance/Bybit SPOT pilot lane.",
      );
    }
    ...
    const id = `timing-${hash({routeKey: route.routeKey, evidenceHash}).slice(0, 32)}`;
    ...
    const requiredApprovalPhrase = `APPROVE ${id}`;
    const record = freeze({
      ...
      publicSamples: Math.min(
        ...
        route.metrics.decisionToExecutionStartMs.sampleCount,
      ),
  ...
  approve(
    ...
    const approved = freeze({
      ...
      approvedAt: now,
      expiresAt: now + this.maximumApprovalDurationMs,
    });
