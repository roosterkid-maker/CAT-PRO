...
export class StrategyOneOrderTimeSafetyService {
  ...
  evaluate(input: {
    ...
    return {
      ...
      reasons:
        approved
          ? [
              `Fresh two-book FOK last-look remains ${postStressNetProfitPercent?.toFixed(4)}% net after fees, adverse-move reserve and safety buffer.`,
            ]
          : [
              ...new Set(
                reasons,
  ...
  private validateVenueContract(
    ...
    if (
      ...
      reasons.push(
        `${side} venue ${exchange} has no audited ${this.config.requiredTimeInForce} mapping.`,
      );
    }
