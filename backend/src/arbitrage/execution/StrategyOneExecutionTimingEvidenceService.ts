...
export class StrategyOneExecutionTimingEvidenceService {
  ...
  observePaperStage(snapshot: OpportunitySnapshot, stage: StrategyOnePaperTimingStage, observedAt = Date.now()): void {
    ...
    for (const opportunity of opportunities) {
      try {
        const stageDuration = observedAt - snapshot.generatedAt;
        const buyQuoteAge = observedAt - opportunity.pair.buy.timestamp;
        const sellQuoteAge = observedAt - opportunity.pair.sell.timestamp;
        const values = stage === "PIPELINE_START" ? [stageDuration, buyQuoteAge, sellQuoteAge] : [stageDuration];
        if (values.some((value) => !validDuration(value))) {
          this.invalidSamplesRejected += 1;
          continue;
        }
        const route = this.ensureRoute(opportunity, observedAt);
        const lastCapturedAt = route.lastStageCapturedAt[stage];
        if (lastCapturedAt !== undefined && observedAt - lastCapturedAt < this.minimumRouteSampleIntervalMs) continue;
        ...
        if (stage === "PIPELINE_START") {
          ...
          this.recordVenueQuoteAge(opportunity.pair.buy.exchange, buyQuoteAge, observedAt);
          this.recordVenueQuoteAge(opportunity.pair.sell.exchange, sellQuoteAge, observedAt);
        } else {
          const metric = stage === "QUEUE_READY" ? "decisionToQueueMs"
            : stage === "EXECUTION_START" ? "decisionToExecutionStartMs" : "decisionToPaperCompletionMs";
  ...
  private routeReport(route: MutableRoute, venues: ReadonlyMap<string, StrategyOneVenueTimingReport>): StrategyOneRouteTimingReport {
    ...
    const publicTimingReady = route.paperSnapshots >= this.minimumPublicSamples && publicSpan >= this.minimumObservationSpanMs &&
      ...
      metrics.decisionToExecutionStartMs.sampleCount >= this.minimumPublicSamples;
    ...
    if (publicTimingReady) {
      const required = Math.ceil(Math.max(metrics.buyQuoteAgeMs.p99Ms ?? Number.POSITIVE_INFINITY,
        ...
        (metrics.decisionToExecutionStartMs.p99Ms ?? Number.POSITIVE_INFINITY) + this.advisorySafetyMarginMs,
  ...
  private ensureRoute(opportunity: ArbitrageOpportunity, observedAt: number): MutableRoute {
    return this.ensureRouteIdentity(opportunity.pair.market, opportunity.pair.buy.exchange,
      opportunity.pair.sell.exchange, observedAt);
  }
