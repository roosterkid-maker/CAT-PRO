...
- Route selection now prefers a currently legal `READY` alternative over a higher-ranked route blocked only by exchange order rules. The shared quantity normalizer also tolerates insignificant IEEE-754 noise at an exact minimum-notional boundary without allowing a genuinely undersized order.
- No capital cap, profit threshold, fee/TDS rule, balance, credential, API permission, exchange setting, fund, order, transfer, withdrawal, evidence or history changed. Local deterministic verification passed `208/208`; backend build, frontend lint and frontend production build passed.
- Deployment was guarded and proceeded only after the operator disarmed the existing Tiny-LIVE arm and account lease. Post-deploy backend/frontend/gateway are healthy, edge is running, internal `/bot` returns HTTP `200`, deployed backend/frontend source markers are present, and active arm/lease are both absent.
- The Tiny-LIVE-capable runtime environment remains `TRADING_MODE=live`, `TRADING_EXECUTION_MODE=live`, `LIVE_TRADING_ENABLED=true`, but no durable arm or account lease exists, so this deployment did not authorize or submit an exchange order. Point-in-time V87 truth after deployment was `READY` for current `COTIUSDT` CoinDCX→Binance inventory and Personal Capital Manager `READY_FOR_PREFLIGHT`; this is readiness, not a fill or profit claim.
- Sanitized artifact: `.deploy/cat-pro-v197-inventory-legality-20260825.tgz`, SHA-256 `012F6730081EF50DDBB895FE1E2FB90912DC2B0E5C7A452C0A2A9E0E0D7C7F39`. AWS source backup: `/opt/cat-pro/.deploy/backups/v197-inventory-legality-20260825/source-before.tgz`; rollback image tags: `cat-pro-backend:pre-v197-inventory-legality` and `cat-pro-frontend:pre-v197-inventory-legality`.
...
- The retired fixed seven-coin arm is no longer accepted or restored. Controlled batch arming now uses the dynamic USDT route pool, with every candidate independently revalidated against current policy, venue contracts, inventory, timing, depth, fees and final last-look gates.
...
## V191-V191.1 Funding Rate, Strategy #3 and Strategy #4 PAPER/SHADOW repair (2026-08-24)

- The supplied Hummingbot `v2_funding_rate_arb.py` reference was treated as design input, not executable authority. Strategy #5 now evaluates both valid funding structures: cross-exchange matched perpetual long/short and intra-exchange cash-and-carry (`LONG spot + SHORT perpetual`). Same-venue perpetual long/short is deliberately excluded because its funding legs cancel economically.
- Funding rates are normalized by each venue's actual interval. Entry VWAP uses genuine full depth; common executable quantity uses a decimal least-common increment; entry/exit fees, adverse slippage, safety reserve and actual notional funding cash flows remain explicit. Long and short funding events have independent timestamp schedules, so different funding clocks do not create false synchronized-payment assumptions.
- Strategy #5 is enabled only in the observation-only SHADOW allow-list. It is not opted into the central PAPER worker and receives no LIVE/order authority. The deployed controller is running and evaluated `9` genuine routes; `6` passed the raw funding-differential threshold, but `0` were net-positive after bounded costs and `0` qualified. The current best intra Binance BTCUSDT route was approximately `-0.4083%` expected net; the best cross Binance/Bybit route was approximately `-0.3682%` expected net. No signal or profit was fabricated.
- Strategy #3 ACLA had a persisted SHADOW cycle stranded at `LEG_3_FILLED` after an earlier process restart. V191.1 recognizes only an exact three-leg, simulated, order-ID-free persisted cycle, reconstructs the final start-asset quantity, conservatively assigns unverifiable positive P&L to dust and completes the ledger idempotently. Production reconciled the old cycle to `COMPLETED`, booked `₹0` realized profit, retained `₹3.49148295` as dust, cleared `openCycleId`, and preserved the `₹1,000` strategy-capital invariant. No account or exchange order mutation occurred.
- Strategy #3 is running in SHADOW. The current scan evaluated `184` paths, including `16` gross-positive paths, but the current complete economics count is `0`; the dominant blocks are fast-screen gross edge, invalid/start-asset paths and one stale-leg path. This is genuine waiting evidence, not a stopped controller.
- Strategy #4 now normalizes the shared spot/perpetual quantity with the same exact common-increment helper, removing false incompatibility when valid venue steps are not directly divisible. It is running and evaluated `48` routes; `18` had complete economics, `0` were net-positive and `0` qualified. The best current route was approximately `-0.5105%` expected net after costs.
- Strategy #4/#5 remain fail-closed on external derivative evidence: Binance USDM authenticated credentials are not configured; Bybit authenticated reads work but the observed account value is below the configured `1,000` quote target; CoinDCX derivative read returned HTTP `500`; CoinSwitch futures margin evidence is invalid; ZebPay positions returned HTTP `400`. These require future operator/account/API decisions and were not bypassed or modified.
- Backend TypeScript build, frontend lint/build, focused Strategy #3/#4/#5 image tests and the complete deterministic suite passed `207/207`. Real exchange APIs and confirmation-sensitive order tests are outside that deterministic suite.
- V191 artifact: `.deploy/cat-pro-v191-funding-strategy-repair-20260824.tgz`, SHA-256 `18D60F9B9B29161653DB93FD2F6CA6FA4374D5F9338F04DFDFBC7CB6E63EE4C2`. V191.1 artifact: `.deploy/cat-pro-v191.1-acla-restart-reconciliation-20260824.tgz`, SHA-256 `2E910B1F66BDF188EE0AB307042A04885CD15F9AFC1ACC1BF329C1E84191987C`.
- V191.1 rollback source: `/opt/cat-pro/.deploy/backups/v191.1-acla-restart-20260824/source-before.tgz`; rollback image: `cat-pro-backend:pre-v191.1-acla-restart`. Deployed backend image: `sha256:5ada7444471b737b8a0a2e2448a1bc95124c012b2a81d18d16ac0dc93f106199`.
- Post-deployment backend/frontend/gateway are healthy with restart count `0`; edge is running with restart count `0`. The host's pre-existing global environment is `live/live/true`, but the final Tiny-LIVE state has active arm `null`, active account lease `null` and trigger-in-progress `false`. Every new Strategy #3/#4/#5 report explicitly keeps LIVE execution and order submission false. No order, transfer, withdrawal, balance, API permission, credential or history mutation was performed.
- PAPER/SHADOW results are engineering evidence only and are not LIVE-profit proof.

## V192 Strategy #3/#4/#5 priority and coverage audit (2026-08-24)

- Strategy #3's affected-market lane now runs at a bounded `50 ms` refresh instead of `250 ms`, validates that its refresh is faster than the configured opportunity-freshness window, normalizes lower-case exchange allow-lists correctly, and uses indexed path membership rather than repeated nested scans. Both full and affected evaluations are deterministic best-first: the strongest stress-qualified route is retained when signal capacity is bounded, independent of source ordering.
- Strategy #4 and Strategy #5 now expose qualified routes in deterministic expected-net-first order. Their economics, fee, funding, slippage, safety and evidence gates were not relaxed. Strategy #5's default bounded market basket now shares the current derivative candidate registry instead of being limited to BTC/ETH/SOL, allowing genuine COTI and other configured evidence to be evaluated without creating unbounded market fan-out.
- The post-deployment Strategy #3 snapshot was current and controller-running: `6` paths evaluated, `1` gross-positive, `0` economically complete/net-positive/qualified. Its dominant blocker was `FAST_SCREEN_GROSS_EDGE_NOT_MET`; ACLA remained SHADOW-only with no account mutation or order authority.
- The post-deployment Strategy #4 snapshot evaluated `48` routes, `6` with complete economics, `0` gross-positive/net-positive/qualified. The best observed route was about `-0.9032%` expected net. Its truthful blockers remain missing spot-book/spot-fee capability evidence and incomplete authenticated derivative evidence; no negative route was promoted.
- Strategy #5 produced a genuine current COTIUSDT Bybit-long/Binance-short funding candidate: `12` routes evaluated, `7` funding-differential-qualified, `7` economically evaluable, `1` net-positive and `1` qualified. The modeled expected net was about `0.3606%` after entry basis, explicit round-trip fees, slippage and safety reserve, using four hourly Bybit events and one Binance event on their actual schedules. This is a projected funding-carry signal, not guaranteed realized profit.
- Strategy #5 remains safely `DERIVATIVE_EVIDENCE_BLOCKED`: Binance USD-M private credentials are absent, Bybit authenticated reads are healthy but the observed account value is below the unchanged `1000` quote target, and no two-venue PAPER-ready margin pair exists. The controller may observe the signal, but central PAPER admission, LIVE execution and order submission remain false.
- The complete deterministic suite passed `207/207`; real exchange APIs and confirmation-sensitive order tests remain outside that suite. V192 artifact: `.deploy/cat-pro-v192-strategy-345-audit-20260824.tgz`, SHA-256 `501E288BD5D17908769CEDDCD870BB62A2BB942EE3D815F068F7A70FAAB1D376`. Rollback source: `/opt/cat-pro/.deploy/backups/v192-strategy-345-20260824/source-before.tgz`; rollback image: `cat-pro-backend:pre-v192-strategy-345`.
- Deployed backend image: `sha256:1338744891820aa9cc34716fc8ed31fbc7ff2d8104451ef3b0d1508f567ef1dc`. Backend/frontend/gateway are healthy with restart count `0`; edge is running with restart count `0`; no fatal/unhandled/OOM entry was found in the post-deployment log window.
- The host's pre-existing global environment remains `live/live/true`, but the final active arm, account lease and trigger are all absent. Strategies #3/#4/#5 explicitly remain read-only SHADOW/PAPER evidence with `liveExecutionAllowed=false` and `orderSubmissionAllowed=false`. No order, transfer, withdrawal, balance, credential, API permission, evidence or history mutation occurred.

## V193 reconciliation stability hotfix (2026-08-24)

- The production backend restart shown by the temporary `Strategy evidence unavailable` / `OFFLINE` UI was a real unhandled reconciliation race, not a Strategy #5 economics decision. `ExecutionReconciliationEngine.scan()` snapshotted lifecycle records, awaited network reads, and could then revisit a terminal record already removed by bounded-history trimming. The second lookup threw `Order lifecycle record not found`, and the interval's unhandled promise rejection stopped the Node process.
- The reconciler now treats only that exact post-snapshot disappearance as a benign bounded-history race, logs it and continues. All other reconciliation exceptions still fail closed. The timer callback also catches and reports unexpected scan rejection so a diagnostic fault cannot terminate the whole backend.
- A deterministic regression removes a lifecycle record between the scan snapshot and reconciliation, proves the scan completes without a crash, and proves the re-entrancy guard is released. Backend build, the focused regression and the complete deterministic suite passed `208/208`; real exchange/order tests remain excluded.
- Sanitized artifact: `.deploy/cat-pro-v193-reconciliation-stability-20260824.tgz`, SHA-256 `63BE101117AE99B32B5C55C26BBD4DFB12586221C47E3FC86BB9A7C7210FD114`. The first deployment verifier used host `node`, which is not installed, so it automatically rolled back without changing runtime. The corrected verifier ran inside the backend container and the final deployment completed successfully.
- Post-deployment stability: backend, frontend and gateway are healthy with restart count `0`; edge is running with restart count `0`. A 15-minute log check found no recurrence of `Order lifecycle record not found`, unhandled/uncaught exception or fatal entry. The host's pre-existing environment remains `live/live/true`; active Tiny-LIVE arm and account-mode lease remain absent, so this hotfix granted no order authority and moved no funds.
- Fresh Strategy #5 truth at `2026-08-24`: controller running, `12` routes evaluated, `8` differential-qualified, `10` cost-evaluable, `1` net-positive and `1` currently qualified. The current COTIUSDT Binance-long/Bybit-short model is approximately `+0.1054%` expected net after modeled fees, slippage and safety reserve. It is still `DERIVATIVE_EVIDENCE_BLOCKED`: the dedicated Binance USD-M key is absent and Bybit authenticated account value is about `17.96` versus the unchanged `1000` target; therefore PAPER admission, LIVE execution and order submission remain unavailable. This modeled signal is not realized-profit proof.
